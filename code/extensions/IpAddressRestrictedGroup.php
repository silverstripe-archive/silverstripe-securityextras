...
class IpAddressRestrictedGroup extends DataExtension
{
     ...
     */
    public function allowedIPAddress($ip)
    {
        if (!$this->owner->IPRestrictions) {
            return true;
        }
        if (!$ip) {
            return false;
        ...
        foreach ($ipPatterns as $ipPattern) {
            ...
            } elseif (preg_match('/^([0-9]+\.[0-9]+\.[0-9]+)\/24$/', $ipPattern, $matches)
                || preg_match('/^([0-9]+\.[0-9]+)\/16$/', $ipPattern, $matches)
                || preg_match('/^([0-9]+)\/8$/', $ipPattern, $matches)) {
                if (substr($ip, 0, strlen($matches[1])) == $matches[1]) {
                    return true;
                }
            }
        }
